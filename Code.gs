...
const ACTIVE_JOB_COLS = ['OPERATOR','ITEM','OPERATION','START_TS','TOKEN'];
...
function doGet(e) {
  try {
    ...
    if (action === 'getAll')  return makeResponse({ ok:true, jobs:readSheet('Jobs',JOB_COLS), timeLogs:readSheet('Timings',TIME_COLS), activeJobs:readSheet('ActiveJobs',ACTIVE_JOB_COLS) });
...
function doPost(e) {
  try {
    ...
    switch(data.action) {
      ...
      case 'updateStatus': var sres=updateStatus(data.item || data['Item Desc'], data.status); return makeResponse({ok: sres==='ok', result: sres});
      case 'deleteJob':    deleteRow('Jobs',    'Item Desc', data.item);                                return makeResponse({ok:true});
      case 'deleteLog':    deleteLogRow(data.operator, data.drgNumber, data.operation, data.date); return makeResponse({ok:true});
      case 'updateLog':    updateLogRow(data); return makeResponse({ok:true});
      case 'addManualLog': appendRow('Timings', data, TIME_COLS); return makeResponse({ok:true});
      case 'saveActiveJob': saveActiveJob(data); return makeResponse({ok:true});
      case 'clearActiveJob': clearActiveJob(data.operator); return makeResponse({ok:true});
...
function deleteRow(sheetName, matchCol, matchVal, matchVal2, matchVal3) {
  ...
  for (var i=rows.length-1;i>=0;i--) {
    if (String(rows[i][mc]).trim()===String(matchVal).trim()) { sh.deleteRow(i+2); return; }
  }
}

// Precise 4-field log delete: OPERATOR + DRG NUMBER + OPERATION + DATE
function deleteLogRow(operator, drgNumber, operation, date) {
  var ss=SpreadsheetApp.getActiveSpreadsheet(), sh=ss.getSheetByName('Timings');
  if (!sh||sh.getLastRow()<2) return;
  var lc=sh.getLastColumn(), lr=sh.getLastRow();
  var hdrs=sh.getRange(1,1,1,lc).getValues()[0];
  var opC =hdrs.indexOf('OPERATOR');
  var drgC=hdrs.indexOf('DRG NUMBER');
  var oprC=hdrs.indexOf('OPERATION');
  var datC=hdrs.indexOf('DATE');
  var tz  =Session.getScriptTimeZone();
  var rows=sh.getRange(2,1,lr-1,lc).getValues();
  for (var i=rows.length-1;i>=0;i--) {
    var rowOp  = String(rows[i][opC] ||'').trim();
    var rowDrg = String(rows[i][drgC]||'').trim();
    var rowOpr = String(rows[i][oprC]||'').trim();
    var rowDat = rows[i][datC] instanceof Date
      ? Utilities.formatDate(rows[i][datC],tz,'dd/MM/yyyy')
      : String(rows[i][datC]||'').trim();
    if (rowOp===String(operator||'').trim() &&
        rowDrg===String(drgNumber||'').trim() &&
        rowOpr===String(operation||'').trim() &&
        rowDat===String(date||'').trim()) {
      sh.deleteRow(i+2);
      Logger.log('deleteLogRow: deleted row '+(i+2));
      return;
    }
  }
  Logger.log('deleteLogRow: no match found for op='+operator+' drg='+drgNumber+' opr='+operation+' date='+date);
}

// Update an existing timing log row — matched by original 4 fields, overwrites all TIME_COLS
function updateLogRow(data) {
  var ss=SpreadsheetApp.getActiveSpreadsheet(), sh=ss.getSheetByName('Timings');
  if (!sh||sh.getLastRow()<2) return;
  ensureColumns(sh, TIME_COLS);
  var lc=sh.getLastColumn(), lr=sh.getLastRow();
  var hdrs=sh.getRange(1,1,1,lc).getValues()[0];
  var opC =hdrs.indexOf('OPERATOR');
  var drgC=hdrs.indexOf('DRG NUMBER');
  ...
  for (var i=0;i<rows.length;i++) {
    ...
    var rowDat = rows[i][datC] instanceof Date
      ? Utilities.formatDate(rows[i][datC],tz,'dd/MM/yyyy')
      : String(rows[i][datC]||'').trim();
    if (rowOp===String(data._origOperator||'').trim() &&
        rowDrg===String(data._origDrg||'').trim() &&
        rowOpr===String(data._origOperation||'').trim() &&
        rowDat===String(data._origDate||'').trim()) {
      // Overwrite row with new values
      var newRow = hdrs.map(function(h){ var v=data[h]; return (v===undefined||v===null)?'':v; });
      sh.getRange(i+2,1,1,newRow.length).setValues([newRow]);
      Logger.log('updateLogRow: updated row '+(i+2));
      return;
    }
  }
  Logger.log('updateLogRow: no match found — appending as new row');
  appendRow('Timings', data, TIME_COLS);
}


// ════════════════════════════════════════════════════════════
// ACTIVE JOBS — real-time operator status
// ════════════════════════════════════════════════════════════
function saveActiveJob(data) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('ActiveJobs');
  if (!sh) {
    sh = ss.insertSheet('ActiveJobs');
    sh.getRange(1,1,1,ACTIVE_JOB_COLS.length).setValues([ACTIVE_JOB_COLS]);
    styleHeader(sh, 1, ACTIVE_JOB_COLS.length);
  }
  ensureColumns(sh, ACTIVE_JOB_COLS);
  var hdrs = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  var opIdx = hdrs.indexOf('OPERATOR');
  var lastRow = sh.getLastRow();
  // Update if operator row exists, else append
  if (lastRow >= 2) {
    var opVals = sh.getRange(2, opIdx+1, lastRow-1, 1).getValues();
    for (var i = 0; i < opVals.length; i++) {
      if (String(opVals[i][0]).trim() === String(data['OPERATOR']).trim()) {
        var row = hdrs.map(function(h){ var v=data[h]; return (v===undefined||v===null)?'':v; });
...
function clearActiveJob(operator) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName('ActiveJobs');
  if (!sh || sh.getLastRow() < 2) return;
  var hdrs = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  ...
  var lastRow = sh.getLastRow();
  var opVals = sh.getRange(2, opIdx+1, lastRow-1, 1).getValues();
  for (var i = opVals.length-1; i >= 0; i--) {
    if (String(opVals[i][0]).trim() === String(operator).trim()) {
      sh.deleteRow(i+2);
      return;
    }
  }
